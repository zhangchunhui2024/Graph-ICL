...
def prepare_input_boxed(template, input_d):
    ...
    # 如果response超过28000字符，保留最后28000字符
    if len(response) > 28000:
        response = response[-28000:]
...
def load_config_from_json(filename='config.json', item=None):
        with open(filename, 'r') as file:
            ...
            if item == 'model_path':
                return config.get('model_path2', None)
...
def main():
    ...
    # 构建文件路径
    file_path1 = f"/home/zch/Code/GraphICL/result/GraphInstruct/Qwen3-32B/responses_with_context_all_tasks/responses_with_context_all_tasks_{result_file}.json"
    ...
    # 构建文件路径
    file_path2 = f'/home/zch/Code/GraphICL/result/GraphInstruct/Qwen3-32B/ICLtask_accuracies/ICLtask_accuracies_{result_file}.txt'
