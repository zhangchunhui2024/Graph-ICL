...
def main():
    ...
    for question_task in group_mapping[question]:
        print(f"Processing question: {question_task}")
        for graph_type in graph_types:
            # 直接从预处理的数据结构中获取 task_data
            task_data = preprocessed_data[question_task].get(graph_type, [])
            # 检查 task_data 是否为空列表
            if not task_data:
                continue
            for group in group_types:
                ...
                all_examples = []
                for example_task in group_mapping[group]:
                    examples = preprocessed_data[example_task].get(graph_type, [])
                    all_examples.extend(examples)
                # 检查 all_examples 是否为空列表
                if not all_examples:
                    continue
                prompt_token_ids = []
                resps = []
                for e in task_data:
                # 随机抽取上下文示例，排除当前问题
                    filtered_examples = [item for item in all_examples if item != e]
                ...
                # 调用模型生成答案
                print(f"Generating answers for task: {group}  graph: {graph_type}")
    ...
    filename = f'/home/zch/Code/GraphICL/result/GraphSCB/test3/responses_with_context_all_tasks/responses_with_context_all_tasks_{question}_{template_icl}_{timestamp}.json'
