...
def main():
    ...
    # 构建文件路径
    file_path1 = f"/home/zch/Code/GraphICL/result/GraphSCB/test3/responses_with_context_all_tasks/responses_with_context_all_tasks_{result_file}.json"
    ...
    for item in input_data['train']:
        for group in group_types:
            if item['task'] in group_mapping[group]:
                task = (group, item['category'])
        if task not in task_groups:
            task_groups[task] = []
        task_groups[task].append(item)

    # 构建文件路径
    file_path2 = f'/home/zch/Code/GraphICL/result/GraphSCB/test3/ICLtask_accuracies/ICLtask_accuracies_{result_file}.txt'
