...
def load_config_from_json(filename='config.json', item=None):
        with open(filename, 'r') as file:
            ...
            elif item == 'example_difficulty':
                return config.get('example_difficulty', None)
...
def main():
    ...
    example_difficulty = load_config_from_json(item='example_difficulty')  # 使用配置文件中的示例难度
    print(f"Processing question: {example_difficulty}")
    for task_type in task_types:
        print(f"Processing task: {task_type}")
        # 示例池
        example_data = [item for item in input_data['train'] if item.get('task') == task_type and item.get('complexity') == example_difficulty ]
        for graph_type in graph_types:
            task_data = [item for item in input_data['train'] if item.get('task') == task_type and item.get('graph') == graph_type ]
            # 检查 task_data 是否为空列表
            if not task_data:
                continue
            # 过滤后的示例池
    ...
    filename = f'/home/zch/Code/GraphICL/result/GraphSCB/test2/responses_with_context_all_tasks/responses_with_context_all_tasks_{context_size}_{example_difficulty}_{template_icl}_{timestamp}.json'
