...
def load_config_from_json(filename='config.json', item=None):
        with open(filename, 'r') as file:
            ...
            if item == 'model_path':
                ...
                return config.get('timestamps', None)
...
def main():
    ...
    timestamps = load_config_from_json(item='timestamps')  # 使用配置文件中的时间戳

    # 构建文件路径
    file_path1 = f"/home/zch/Code/GraphICL/result/GraphSCB/responses_with_context_all_tasks/responses_with_context_all_tasks_{timestamps}.json"
    ...
    for item in input_data['train']:
        task = (item['task'], item['complexity'])
        if task not in task_groups:
            task_groups[task] = []
        task_groups[task].append(item)

    # 构建文件路径
    file_path2 = f'/home/zch/Code/GraphICL/result/GraphSCB/ICLtask_accuracies/ICLtask_accuracies_{timestamps}.txt'
