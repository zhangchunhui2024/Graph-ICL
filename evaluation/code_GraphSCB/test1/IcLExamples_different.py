...
def main():
    ...
    for task_type in task_types:
        ...
        # 示例池
        example_data = [item for item in input_data['train'] if item.get('task') == task_type and item.get('complexity') == "easy" ]
        for complexity_type in complexity_types:
            # 难度不同示例池不同
            if complexity_type == "easy":
                for graph_type in graph_types:
                    task_data = [item for item in input_data['train'] if item.get('task') == task_type and item.get('complexity') == complexity_type and item.get('graph') == graph_type ]
                    # 检查 task_data 是否为空列表
                    if not task_data:
                        continue
                    # 过滤后的示例池
                    filtered_examples = [item for item in example_data if item.get('graph') != graph_type]
                    prompt_token_ids = []
                    resps = []
                    for e in task_data:
                    # 随机抽取上下文示例
                    ...
                    # 调用模型生成答案
                    print(f"Generating answers for task: {task_type}  complexity: {complexity_type} graph: {graph_type}")
            ...
            elif complexity_type == "middle" or complexity_type == "hard":
                task_data = [item for item in input_data['train'] if item.get('task') == task_type and item.get('complexity') == complexity_type ]
                # 示例池
                filtered_examples = example_data
                prompt_token_ids = []
                resps = []
                for e in task_data:
                # 随机抽取上下文示例
                    ...
                    prompt_token_ids.append(tokenized_prompt)
                # 调用模型生成答案
                print(f"Generating answers for task: {task_type}  complexity: {complexity_type}")
                generations = llm.generate(prompt_token_ids=prompt_token_ids, sampling_params=sampling_params)
                for i in range(len(task_data)):
                    d = task_data[i].copy()
                    generated = generations[i].outputs[0].text.strip()
